...
def check_collision():
    ...
    for egg in eggs:
        if (
            egg[1] + egg_radius > basket_y
            and egg[0] >= basket_x
            and egg[0] <= basket_x + basket_width
        ):
            eggs.remove(egg)
            score += 1
            eggs_caught += 1
...
def display_control_options():
    ...
    screen.blit(
        keys_text,
        (
            (screen_width - keys_text.get_width()) // 2,
            (screen_height - keys_text.get_height()) // 2 - 50,
        ),
    )
    screen.blit(
        mouse_text,
        (
            (screen_width - mouse_text.get_width()) // 2,
            (screen_height - mouse_text.get_height()) // 2 + 50,
        ),
    )
    pygame.display.flip()
    ...
    if game_over:
        ...
        screen.blit(
            ...
            (
                ...
                (screen_height + 20 + score_text.get_height()) // 2,
            ),
        )
        screen.blit(
            replay_text,
            (
                (screen_width + 20 - replay_text.get_width()) // 2,
                (screen_height + 90 + game_over_text.get_height()) // 2,
            ),
        )
        pygame.display.flip()
        for event in pygame.event.get():
